...
rscripts    = "/Volumes/MiniOne/Projects/BonetJaume/RosettaFFL/side_changes/source/bin/rosetta_scripts.macosclangrelease -parser:protocol alignment.xml"
command     = "{0} -s {1} -parser:script_vars template={2} align={3} -out:prefix {4} -out:file:score_only rmsd_evaluation -overwrite"
align_mode  = ["align1", "align2"]

for i in range(len(experiments)):
    queries = [x[1] for x in experiments[i+1:]]
    if len(queries) > 0:
        print experiments[i][1], queries
